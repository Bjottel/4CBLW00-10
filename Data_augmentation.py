...
# Number of augmentations per spectrum
n_aug = 5
...
# Visualise original vs augmented spectra

def plot_original_vs_augmented(x, original_y, augmented_y_list, title_prefix=""):
    ...
    plt.title(f'{title_prefix} Original vs Augmented Spectra')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
df_aug = pd.read_csv('augmented_spectra.csv')
df_aug['spectrum'] = df_aug['spectrum'].apply(json.loads)

# Pick a group of augmentations
example_id = df_aug['origin_id'].iloc[0]
example_rows = df_aug[df_aug['origin_id'] == example_id]
example_rows = example_rows.reset_index(drop=True) # ensure the original is first
...
plot_original_vs_augmented(x, original_y, augmented_y_list, title_prefix=f"Origin ID: {example_id}")
